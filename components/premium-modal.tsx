...
export function PremiumModal({ open, onClose, feature }: PremiumModalProps) {
  ...
  const canStartTrial = subscription.tier === 'free' && !subscription.trialEndsAt
  ...
  const price = billingCycle === 'yearly' ? premiumPlan.priceYearly : premiumPlan.price
  const savingsPercent = Math.round((1 - premiumPlan.priceYearly / (premiumPlan.price * 12)) * 100)

  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
      <div className="fixed inset-0 bg-black/50 modal-overlay" onClick={onClose} />
      
      {/* Thay đổi 1: Thêm flex flex-col và đổi sang overflow-hidden */}
      <div className="relative bg-white rounded-t-3xl sm:rounded-2xl max-w-lg w-full max-h-[90vh] flex flex-col overflow-hidden shadow-2xl modal-content">
        ...
        {/* Thay đổi 2: Header thêm flex-shrink-0 để không bị co lại */}
        ...
        {/* Thay đổi 3: Phần nội dung trung tâm thêm flex-1 và overflow-y-auto */}
          ...
          {canStartTrial && (
            <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-xl p-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-green-900 mb-1">Try Premium Free for 7 Days</h3>
            ...
            <div className="text-center mb-6">
              ...
              {billingCycle === 'yearly' && (
                <p className="text-sm text-green-600 mt-1">
                  Just ${(premiumPlan.priceYearly / 12).toFixed(2)}/month billed annually
                </p>
              )}
            </div>
          </div>
        ...
        {/* Thay đổi 4: Phần Footer (CTA & Fine Print) thêm flex-shrink-0 và padding */}
        <div className="flex-shrink-0 p-6 pt-4 border-t bg-white">
          <div className="space-y-3">
            ...
            >
              {isProcessing ? 'Processing...' : `Upgrade to Premium - $${price}`}
            </Button>
